    ...
    for (int i = 0; i < height; ++i) {
        for (int j = 0; j < width; ++j) {
            ...
            if (oneCellBugs.size() > 1){
                Bug* maxBug = *max_element(oneCellBugs.begin(), oneCellBugs.end(), [](Bug* a, Bug* b) { return a->getSize() < b->getSize(); }); //Finding the biggest bug, Eat the smallest bug
                vector<Bug*> maxBugs;
                ...
                if(maxBugs.size() > 1){ //Checking biggest bug between two bugs
